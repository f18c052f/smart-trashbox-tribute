...
[requirements.md NFR-4](./requirements.md#nfr-4-制御周期暫定目標) は、必要 fps を
**「必要サンプル数 ÷ 許容時間」から導出する**と定めている（固定値を先に置かない）。
...
> データフォーマットは未確定（→ [OQ-32](./open-questions.md#g-観測基盤独自機能)）。
...
（以下は将来案・未作成）
  ...
  通信層        ESP32 への送信
  移動体側      ESP32 ファームウェア
  観測基盤      Throw Record の記録・可視化・評価（別言語想定 → original-features.md §9）
```
...
> 選定条件は [OQ-23](./open-questions.md#e-固定側検出予測計算機) を正とする。
...
上表の項目は「測る」と決まっているだけで、**収集手段が長らく未定義だった**。
[original-features.md 柱2a](./original-features.md#41-2a-構造化ロギング--これは避けられない) で
**構造化ロギング**として実装する方針を採る（**リアルタイム送信・ダッシュボードは作らない**）。

- Pi 側: 各段階の時刻を構造化ログとしてファイルへ追記し、**後から集計する**
- ESP32 側: 走行中は RAM にバッファし、**走行終了後に吸い出す**

> M1・M2 で見たい現象（0.4〜0.9秒の挙動、発進時スリップ、急加速時の電圧降下）は
> **人間がリアルタイムで見るには速すぎる。** 必要なのは後から拡大して読める記録であって、
> 監視画面ではない。
...
| ID | 事項 |
|---|---|
| [OQ-23](./open-questions.md#e-固定側検出予測計算機) | Pi 4 の OS（RealSense 導入容易性で判断） |
| [OQ-24](./open-questions.md#e-固定側検出予測計算機) | Pi 4 の RAM 容量 |
| [OQ-25](./open-questions.md#e-固定側検出予測計算機) | RealSense の解像度・fps 設定（→ §5） |
| [OQ-26](./open-questions.md#e-固定側検出予測計算機) | 物体検出方式（→ §10） |
| [OQ-27](./open-questions.md#e-固定側検出予測計算機) ★ | **Pi 4 で必要性能を達成できるか**（→ §13） |
...
**現時点で未確認のものを確定扱いにしない。** Pi 4 と RealSense の実機セットアップ時に、以下を**この順で**確認する。

| # | 確認事項 | 対応 |
|---|---|---|
| 1 | Raspberry Pi 4 の **RAM 容量** | [OQ-24](./open-questions.md#e-固定側検出予測計算機) |
| 2 | **Raspberry Pi OS / Ubuntu** の最終選定 | [OQ-23](./open-questions.md#e-固定側検出予測計算機) |
| 3 | RealSense D435 の Pi 4 での**認識** | [OQ-28](./open-questions.md#e-固定側検出予測計算機) |
| 4 | **USB3 接続**確認 | 同上 |
| 5 | RealSense **電源安定性** | 同上 |
| 6 | **pyrealsense2 / librealsense** 導入方法 | 同上（結果が #2 の判断材料になる） |
| 7 | **640×480 30fps** / **60fps** 動作確認 | [OQ-25](./open-questions.md#e-固定側検出予測計算機) |
| 8 | **CPU 使用率 / メモリ使用率 / dropped frame / end-to-end latency** | [§13.1](#131-実測する項目) → [OQ-27](./open-questions.md#e-固定側検出予測計算機) |

> ⚠️ **#3〜#6 を #7 より先に行う。** RealSense は USB3 帯域と給電の影響を受けやすく、
> ここが不安定なまま fps を測ると**「Pi 4 の性能不足」と誤診する。**

> #8 の測定は [§13.1](#131-実測する項目) の実測項目そのものであり、
> **Pi 4 を継続するかの判断（[§13](#13-raspberry-pi-4-を継続するかの判断)）に直結する。**
