...
> ⚠️ **ライセンス**: 本リポジトリは包括的な利用許諾を置いていない。とくに手動テレオペ用ファームウェアには
> 商用利用不可の依存（BTstack）が入る方針であるため、再配布の前に
> [ライセンスと再配布上の制約](#ライセンスと再配布上の制約) を読むこと。
...
⚠️ **本リポジトリはまだ包括的な利用許諾（トップレベルの `LICENSE`）を置いていない。**
...
（`firmware/CMakeLists.txt` の `COMPONENTS` 許可リストによる。
検査は `tests/firmware/test_firmware_boundaries.py` が固定している）。
テレオペ用と本番用のビルドは相互排他であり、両方を同時に指定するとコンパイルが失敗する。
