...
class BracketMeasurements:
    ...
    Attributes:
        outline_x_mm: ブラケット外形のうち**モータ軸方向**（＝機体の半径方向）の
            寸法（mm）。⚠️ **2値のうち長いほうがこちらである**——`docs/bom.md §B`
            が「モータ軸方向に長い」と述べており、モータ軸は
            `mount_face_to_wheel_center_mm` を測った向きそのものである。
        outline_y_mm: ブラケット外形のうち**モータ軸に直交する向き**（＝機体の
            接線方向）の寸法（mm）。⚠️ **アームの幅（`base.arm_width_mm`）へ載る
            のはこちらである。** 取り違えれば、載るかどうかの判断が別の量について
            の判断にすり替わる。
        ...
        mount_hole_count: ⚠️ **締結に使う穴の数**（ブラケットが持つ穴の数ではない）。
            1 以上。⚠️ **現物のブラケットは穴を4つ持ち、22.9mm × 29.6mm の長方形に
            並んでいる**（2026-09-10 実測）。そのうち⚠️ **接線方向の2つだけを使う**
            ——半径方向にも留めるとバッテリトレイの耳がアームの外端を越えるため
            である（取付面→ホイール中心 33.9mm から長穴の半分 3.7mm を引いた帯に、
            耳は座の外径 9.2mm ＋ 耳の長さ 18.4mm を要する。⚠️ **許される半径
            ピッチの上限は 5.2mm** であり、現物の 22.9mm は入らない）。
            ⚠️ **この値を「ブラケットが持つ穴の数」と読まないこと**——2026-09-03 の
            記録は写真からの目視で「穴が2つ見える」としており、それが誤りであった。
        mount_hole_diameter_mm: 取付穴の径（mm）。⚠️ 現物で確かめたのは
            **M3 が通ること**であり、この数値そのものではない（出所は仮値）。
        mount_hole_pitch_mm: ⚠️ **締結に使う2穴の中心距離**（mm）。⚠️ **接線方向の
            ピッチである**——長穴は半径方向に伸び、この値は長穴どうしの接線方向の
            間隔を決める。⚠️ 現物のもう一方のピッチ 22.9mm（半径方向）は使わない。
