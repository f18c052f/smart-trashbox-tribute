...
### Decision: 無線ライブラリのライセンス表明の出典

- **Context**: 要件 1.6 が配布時に誤読されない形の記載を求める。配布者向けの記載である以上、
  ライセンス表明そのものに出典が要る
- **出典**: `.kiro/steering/roadmap.md`「ファームウェア構成の決定（2026-08-23 の技術検証で確定）」の
  ライセンス節 — **Bluepad32 本体は Apache-2.0 だが、依存する BTstack はオープンソースではない**
  （「個人的利益のためのみ。商用目的・金銭的利得のためには使用不可」条項付き。商用は
  BlueKitchen 社の有償ライセンス）
- **Implications**: 本 Spec の記載はこの調査結果を引き写すものであり、独自の法的判断を含まない。
  ⚠️ **encumbrance をテレオペビルドへ封じ込めれば本番成果物はクリーンに保てる**という結論が、
  本番から無線を除外する第2の根拠（2.4GHz 共存とは別）になっている
