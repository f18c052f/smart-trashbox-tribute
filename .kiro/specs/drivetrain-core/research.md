...
### Decision: 開ループ入口は本 Spec が所有し、分岐を `applyWheelOutputs()` 内へ閉じる（2026-08-24 追加）

- **Context**: 本 Spec の実装完了・マージ後、`teleop-bringup` の要件生成中に齟齬が判明した。`docs/requirements.md` の **M2a-1「接地・オープンループ（PWM 直接指令）で手動走行」**が、本 Spec の既存2入口では実現できない（どちらも必ず速度PID を経由する）。M2a-1 は「エンコーダ符号が逆でも PID が暴走しない状態で接地挙動を確かめる」という**安全上の段階分け**であり、省略できない
- **Alternatives Considered**:
  1. **`teleop-bringup` が `MotorOutputPort` へ直接書く** — 保護①〜④をすべて迂回する。保護一式と `ProtectionSupervisor` は `DrivetrainController` の内部構造として意図的に非公開であり、下流から適用する手段が構造的に無い。下流で保護を再実装するのは `tech.md` 開発標準3（本番アルゴリズムの二重実装の禁止）に反し、かつ「M2a より前に保護①〜④を実装する」（`docs/drivetrain-spec.md` §10）の趣旨を殺す。**接地走行で最も危険な選択肢**
  2. **M2a-1 を廃し M2a-0 → M2a-2 の2段にする** — `docs/requirements.md` の安全な段階分けを壊す。接地して最初に速度PID を回すことになり、エンコーダ符号が逆なら初回接地で PID が発散する
  3. **保護一式を公開して下流に組み立てさせる** — 公開面が大きく広がり、`DrivetrainController` が「内部構造を隠した合成器」である意味が失われる。下流ごとに組み立て順序を誤る余地が生まれる
- **Selected Approach**: 第4の指令入口 `submit(WheelDutyCommand)` を本 Spec へ追加する。`WheelTargets` に `ControlPath path` と `duty[]` を持たせて**既存と同一のラッチへ落とし**、分岐は `DrivetrainController::applyWheelOutputs()` の**内部だけ**に置く。`Lock` 以降（`Sup` → `Gate` → `Polarity` → `Write`）は一切変更しない
- **Rationale**: 要件 5.9 に対して既存設計が採った「直接入口＋保護の同一適用を**構造で**保証する」型をそのまま踏襲できる。`applyWheelOutputs()` の外側が経路を知らないため、要件 5.11（開ループでも保護を同一適用）が実装の注意深さに依存しない。ラッチを1本に保つことで、ウォッチドッグ（要件 5.12）が「どちらのラッチの時刻を見るか」という分岐を持たずに済み、④を「指令元固有の事情を含まない形」に保つ要件 13.2 / 13.7 とも整合する
- **Trade-offs**: 公開型 `WheelTargets` / `DrivetrainStatus` が広がる（`duty[]` / `path` / `control_path`）。`path` に応じて `mm_s[]` と `duty[]` のどちらが有効かが変わるため、**両方を同時に意味のある値として読めない**という規約が増える。ただし読み手は `applyWheelOutputs()` と `status()` の2箇所に限られる
- **Follow-up**: 経路切替時の PID 積分項の持ち越し（要件 5.14）は、**開ループ中に毎ステップ `holdReset()` を呼ぶ**ことで追加機構なしに解決する。「切替を検出してクリアする」明示的な機構を設けないのは、検出漏れが静かな不具合になる形を避けるためである。この設計判断そのものを検証する統合テスト（積分項を育てる → 開ループ → 復帰）を必須項目として置く
