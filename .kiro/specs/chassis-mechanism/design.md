...
| 6.1, 6.2, 6.9 | 円錐台を受ける座・上流の採寸からの導出と再導出 | Shapes, Config | `AdapterGeometry` / `TrashCanMeasurements` | 寸法 → 形状 |
...
| 6.7 | 開口内径を狭めず受け口と干渉しない | Shapes, test_chassis_invariants | `AdapterGeometry` の不変条件 | 寸法 → 形状 |
| 6.8 | 締結箇所の配置の根拠 | 本書「機構の決定」節, Joints | `JointSpec` | — |
| 7.1, 7.2, 7.3 | バッテリ最下部・着脱・搭載物の拘束 | Shapes, Params | `battery_tray` 部品 | 寸法 → 形状 |
| 7.4, 7.5 | 基板トレイの取付箇所と放熱の隙間 | Shapes, Params | `board_tray` 部品 | 寸法 → 形状 |
...
- 部品群（`wheel_count = 3` の場合）:
  ...
  | `adapter_segment_1..n` | n | 円環（`required_segment_count`） | 6.1, 6.2, 6.5, 6.7, 6.9 |
  | `battery_tray` | 1 | 位相 | 7.1, 7.2, 7.3, 8.5 |
  | `board_tray` | 1 | 位相 | 7.4, 7.5, 8.3, 8.4 |
  | `cable_guide_1..3` | 3 | 位相 | 4.6, 7.6, 7.7, 8.4 |
  | `service_stand_1..3` | 3 | 位相 | 5.1-5.5, 5.8 |

- **不変条件**（`test_chassis_invariants.py` が検査）:
  - アダプタは**円錐台の側面に沿う座**を持ち、⚠️ **円筒断面を持たない**（要件 6.2）
  - アダプタは**ゴミ箱が提供する通過を狭めない**——底の内面（`bottom_outer_diameter_mm/2 − bottom_thickness_mm`）から `taper_deg` で広がる円錐と交わらない（要件 6.7）。⚠️ **`opening_inner_diameter_mm` は下限にならない**。座はφ180 の底を受けるものであり、外径（φ188）ですら開口（φ210）より小さいため、内径がそれを上回る形は存在しない
...
- Postconditions: 成功時は全部品の3形式が存在する。失敗時は出力先が呼び出し前の状態である
...
- **位置**: 基板トレイの側面。⚠️ **ゴミ箱を載せた状態でも外から操作できる高さ**に置く
...
- **後付け余地**: 非常停止手段を後から追加できるねじ穴と配線の引き出しを基板トレイに残す（要件 8.7）
...
- **配置**: 基板トレイの最下段。主ヒューズは**バッテリ直近（端子台より上流）**に置く
...
| 9 | 基板トレイ・端子台・メインスイッチの取り付け | 基板トレイ、端子台、スイッチ | スイッチが外から操作できること |
| 10 | 配線と配線ガイドへの収容 | 配線ガイド | ⚠️ **系統ごとに別経路**。床から離れていること |
| 11 | 最低地上高の実測 | ノギス、平坦な床 | 5部位の隙間を実測（`clearances`。要件 4.5） |
| 12 | 実測重量の測定 | はかり | `mass_g`（要件 10.1） |
| 13 | 重心位置の測定 | 支点、はかり | 手順とともに記録（`cog_*`。要件 10.2） |
| 14 | 荷重下の実効転がり径の測定 | ノギス、平坦な床 | ⚠️ **3個それぞれ**。転動を伴わない場合は限界を注記（`wheels`。要件 10.3-10.5） |
| 15 | 整備スタンドへの載せ降ろし | 整備スタンド3脚 | ⚠️ **一人で実施できること**（要件 5.9）。手で押しても外れないこと（`checks.stand_retention`。要件 5.7） |
| 16 | ゴミ箱固定アダプタの取り付けと着脱 | アダプタ断片、ゴミ箱 | ⚠️ **駆動ベースを分解せずに着脱できること**（要件 6.6）。手で加える力で外れないこと（要件 6.5） |
