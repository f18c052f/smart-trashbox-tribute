...
6. 整備スタンドの支持方式の変更（ホイール外側／駆動ベース端部で支持するという前提）
...
- **不変条件**（`test_chassis_invariants.py` が検査）:
  ...
  - アダプタの内側最小径は上流の `trash_can.opening_inner_diameter_mm` を**下回らない**（要件 6.7）
  - スタンドに載せたとき、3輪の最下点が床面より上にあり、
    かつ**ホイール外周とスタンドの隙間**が確保される（要件 5.4, 5.5）
  - スタンドの支持面はホイール外側または駆動ベース端部にあり、
...
### 決定 5: 整備スタンドは3脚独立とし、ホイール外側で支持する（要件 5.1, 5.3, 5.8, 11.4）

- 支持はホイール外側または駆動ベース端部で行う。
