...
**ステータス: ブロック（design レベルの判断が必要）**

### 実施日

2026-08-27（着手し、実行不能であることを確認した）

### 結論

**`bench-logging` は現在の構造では live に対して実行できない。**
タスク9.5 が要求する「計測比較を**実機で**実行し、3条件を交互に比較する」を
満たすには、3条件比較の構造そのものを design レベルで決め直す必要がある。
...
### 検討しうる方向（design レベルの判断が必要）

いずれも要件 10.1 の解釈に関わるため、**本タスク内で単独に決めない**。

| 案 | 内容 | 論点 |
|---|---|---|
| A | **1本の `FrameSource` を3条件で共有**し、セグメントごとにロガー／レコーダを差し替える | 「同一入力元」を最も文字通り満たす。ただし `CaptureMetrics` は構築時にロガーを束ねるため、ロガーの差し替え口を作る必要があり `metrics` / `source` へ波及する |
| B | セグメントごとに開き直す | 交互実行は保てるが、RealSense の `start()`/`stop()` に**約 0.5 秒**かかる（タスク9.3 の実測）ため、セグメント長を大きくしないと開閉コストが測定値を汚染する |
| C | live では3条件を別々の実行として測り、交互実行を諦める | 順序効果を打ち消せなくなる（Batch/Job Contract の A/B/A/B に反する） |

> ⚠️ **要件 10.5「この比較を、実機を用いずに実行できる入力元に対しても実施
> できるようにする」は現状で満たされている**（simulated / recorded では動作する）。
> 欠けているのは実機での実行であり、それを要求しているのはタスク9.5 である。
