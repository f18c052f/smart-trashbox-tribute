...
### 発見2: live 記録の manifest にデバイス識別情報が無い（**タスク 6.3 / 8.3 で是正。実機確認は未実施**）
...
### 発見3: manifest の `global_time_enabled` が常に `null`（**タスク 8.3 で是正。実機確認は未実施**）
...
#### 是正（タスク 6.3 / 8.3 / 2026-08-28）

**発見2**: `RealSenseSource` が `start()` で実際に開いた個体の識別情報を
`device_identity` として公開し（タスク 6.3）、`cli.run_record()` がそれを
`manifest.json` の `device`（`name` / `serial` / `firmware` / `usb_type` /
`product_line`）へ写すようにした（タスク 8.3）。`probe_devices()` の列挙結果
ではなく**パイプラインが開いた個体**を写す——複数台つながっているときに
別の個体を記録しないため。

**発見3**: `_build_runtime_info()` が入力元を受け取り、live のとき
`RealSenseSource.global_time_enabled` を写すようにした。`True`（試みて成功）
/ `False`（試みて失敗）/ `null`（そもそも試みていない = live 以外）の3値を
区別する。

いずれも live 以外の入力元では `null` のままとし、値を偽装しない（要件 3.5）。
検証は SDK を模したモックで `record` を端から端まで通して行った
（`test_sensing_cli.py::TestRecordManifestMetadata`）。

**実機での確認は未実施**——`device` が実在の個体を指すことは実機でしか
確認できないため、次の実機セッションでタスク 9.5 と同時に行う。

### 未実施の項目

- 発見1〜3 はいずれも是正済み（発見1 はタスク 4.7、発見2・3 はタスク 6.3 / 8.3。
  上記「決着」「是正」節）。ただし**発見2・3 の実機確認は未実施**であり、
  次の実機セッションでタスク 9.5 と同時に行う
